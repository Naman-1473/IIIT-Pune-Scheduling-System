...
const updateCourses = asyncHandler( async ( req, res ) =>
{
    ...
    const updatedcourse = await Course.findOneAndUpdate( {courseId: courseId}, {name: name,instructor:updatedInstructor?._id}, {new: true} )
    if ( !updatedcourse )
    {
        throw new ApiError( 409, "Something went wrong while updating" )
    }
...
const updateSections = asyncHandler( async ( req, res ) =>
{
    ...
    const updatedsection = await Section.findOneAndUpdate( {sectionId: sectionId}, {capacity:capacity}, {new: true} )
    if ( !updatedsection )
    {
        throw new ApiError( 409, "Something went wrong while updating" )
    }
